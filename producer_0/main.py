    ...
    for timestamp in range(150):
        for img_file in glob.glob(f"{data_location}/raw/Cam_*/{str(timestamp)}.png"):
            ...
            with open(img_file, 'rb') as img:
                val = img.read()
                r.set(key, val)
                producer.send(topic_name, key=key.encode(), value=b"contour detection").add_callback(on_send_success).add_errback(on_send_error)
        producer.flush()
        ...
        for event_message in event_consumer:
            ...
            if(event_message.key.decode() == "time_stamp"):
                print(f"Received signal to process next timestamp {event_message.value.decode()}")

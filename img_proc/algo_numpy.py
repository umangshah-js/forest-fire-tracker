...
def prediction(
    ...
    for i in range(burning_centers.shape[0]):
        norm = np.linalg.norm(centers[:, :2] - burning_centers[i, :2], axis=1, ord=2)[
            ..., None
        ]
    ...
    cv2.imwrite(
        save_path, draw_circle(img=img, centers=centers, states=states, thickness=-1)
    )
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(timestamps)):
        npzfile = np.load(
            f"/home/mohit/NYU/Big_Data/Homework/forest-fire-tracker/data/timestamps/arr/{i}.npz"
        )
